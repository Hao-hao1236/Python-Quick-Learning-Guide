...
output("3.2. 元组(tuple)")
output("元组是一种有序的不可变数据结构，可以存储任意类型的元素。")
output("定义元组：\npoint = (3, 4)")
output("元组的元素不能修改，但可以通过索引访问。")
output("示例代码：\npoint = (3, 4)\nx = point[0]  # 获取第一个元素\ny = point[1]  # 获取第二个元素")
output("3.3. 字典(dict)")
output("字典是一种无序的可变数据结构，用于存储键值对。")
output("定义字典：\nstudent = {\"name\": \"Alice\", \"age\": 25, \"is_student\": True}")
output("访问字典：\nname = student[\"name\"]  # 获取键\"name\"对应的值\nage = student[\"age\"]  # 获取键\"age\"对应的值")
output("添加或修改字典：\nstudent[\"grade\"] = \"A\"  # 添加一个新的键值对\nstudent[\"age\"] = 26  # 修改键\"age\"对应的值")
output("删除字典元素：\ndel student[\"is_student\"]  # 删除键\"is_student\"及其对应的值")
output("3.4. 集合(set)")
output("集合是一种无序的可变数据结构，用于存储唯一的元素。")
output("定义集合：\nunique_numbers = {1, 2, 3, 4, 5}")
output("集合的元素不能重复，可以进行集合运算，如并集、交集、差集等。")
output("示例代码：\na = {1, 2, 3}\nb = {3, 4, 5}\nunion = a | b  # 并集\nintersection = a & b  # 交集\ndifference = a - b  # 差集")
output("----------------------------------", delay=5)

output("4. 函数与模块")
output("函数是可重用的代码块，用于执行特定的任务。模块是包含函数和变量的文件，可以被其他Python程序导入和使用。")
output("4.1. 函数")
output("定义函数：\ndef greet(name):\n    return f\"Hello, {name}!\"")
output("调用函数：\ngreeting = greet(\"Alice\")  # 调用函数并传递参数")
output("函数还可以有默认参数、可变参数和关键字参数等高级功能。")
output("示例代码：\ndef greet(name=\"World\", punctuation=\"!\"):\n    return f\"Hello, {name}{punctuation}\"\n\n# 调用函数时使用默认参数\ngreeting1 = greet()  # 输出: Hello, World!\n\n# 调用函数时传递参数\ngreeting2 = greet(\"Alice\")  # 输出: Hello, Alice!\n\ngreeting3 = greet(\"Bob\", \"?\")  # 输出: Hello, Bob?")
output("4.2. 模块")
output("模块是包含函数和变量的文件，可以被其他Python程序导入和使用。")
output("创建模块：\n# 在greetings.py文件中定义一个函数\ndef greet(name):\n    return f\"Hello, {name}!\"")
output("导入模块：\n# 在另一个Python文件中导入greetings模块\nimport greetings\n\ngreeting = greetings.greet(\"Alice\")  # 调用模块中的函数")
output("模块还可以使用from...import语句导入特定的函数或变量，或者使用as关键字为模块或函数指定别名。")
output("示例代码：\n# 导入模块中的特定函数\nfrom greetings import greet\n\ngreeting = greet(\"Alice\")  # 直接调用函数\n\n# 导入模块并指定别名\nimport greetings as gr\n\ngreeting = gr.greet(\"Alice\")  # 使用别名调用函数")
output("----------------------------------", delay=5)

output("恭喜你，掌握了Python的基本语法、数据结构和函数模块！")
output("现在你已经具备了编写Python程序的基础知识，可以开始探索更多的Python功能和库了！")
output("更多可以在同文件夹的shell.py中练习哦~")
output("祝你在Python编程的旅程中取得成功！")
input("按下回车键退出程序...")